...
(function () {
  ...
  var SHORT = { W: '班', R: '休', S: '特', '': '' };
  var FULL = { W: '上班', R: '休息', S: '特休', '': '未選擇' };

  /* ---------- 資料 ---------- */
  function defaultDb() { return { version: 1, employees: [], settings: { perDay: 2 }, months: {} }; }
  function clone(o) { return o == null ? o : JSON.parse(JSON.stringify(o)); }
  function freshDraft() { return Store.freshDraft(); }

  var db = defaultDb();
  var ui = Object.assign({ month: START_KEY, brush: null }, Store.loadUi());
  var downloadsCap = null;   // claude.ai 預覽環境的檔案儲存能力
  var ready = false;
  ...
  function persistUi() { Store.saveUi({ month: ui.month }); }
  ...
  function addEmployee(name) {
    ...
    return go.then(function (yes) {
      if (!yes) return false;
      db.employees.push({ id: uid(), name: name, createdAt: Date.now(), deleted: false, deletedAt: null });
      persist();
      toast('已新增員工「' + name + '」');
      return true;
    });
  ...
  function renameEmployee(id) {
    var e = empById(id);
    if (!e) return Promise.resolve(false);
    return ask({ title: '修改姓名', message: '', input: { value: e.name, placeholder: '員工姓名' }, okText: '儲存' })
      .then(function (name) {
  ...
  function isDirty(key) {
    var m = getMonth(key);
    return canon(m.working) !== canon(m.saved || freshDraft());
  }
  ...
  function cancelMonth(key) {
    var m = getMonth(key);
    var msg = m.saved
      ? '將復原到上次儲存的狀態，之後的所有修改都會遺失。'
      : '本月尚未儲存過，取消將清空本月所有選擇與排班。';
  ...
  function clearPrefs(key) {
    ...
    return ask({ title: '清除選擇', message: '清除本月所有已選的休息與特休？', okText: '清除' }).then(function (yes) {
  ...
  function buildTable(key, opts) {
    ...
    for (d = 1; d <= D; d++) {
      h.push('<th class="day"><span class="dn">' + d + '</span><span class="dw">' + WD[S.weekday(year, month, d)] + '</span></th>');
    }
    if (generated) {
      h.push('<th class="stat">上班</th><th class="stat">休息</th><th class="stat">特休</th><th class="stat" title="累計休息結餘：正值代表至今多休，下月會少排休；負值則下月多排休">結餘</th>');
    } else {
      h.push('<th class="stat">已選休息</th><th class="stat">特休</th><th class="stat" title="過去月份累計的休息結餘">結餘</th>');
    }
    ...
    roster.forEach(function (e) {
      ...
      if (generated) {
        ...
        h.push('<td class="stat"' + tgt + '>' + cnt.W + '</td><td class="stat">' + cnt.R + '</td><td class="stat">' + cnt.S + '</td>' +
          '<td class="stat bal ' + (bal > 0.05 ? 'pos' : bal < -0.05 ? 'neg' : '') + '">' + fmt1(bal) + '</td>');
      } else {
        var bb = balBefore[e] || 0;
        h.push('<td class="stat">' + cnt.selR + '</td><td class="stat">' + cnt.S + '</td><td class="stat bal ' + (bb > 0.05 ? 'pos' : bb < -0.05 ? 'neg' : '') + '">' + fmt1(bb) + '</td>');
      }
      h.push('</tr>');
    });
    h.push('</tbody>');
    ...
    if (roster.length) {
      ...
      for (d = 1; d <= D; d++) {
        if (S.isFixedOff(year, month, d)) { h.push('<td class="wed">–</td>'); continue; }
        var n = generated ? validation.cover[d] : avail.perDayAvail[d];
        var cls2 = n < perDay ? ' class="short"' : (generated && n > perDay ? ' class="over"' : '');
        h.push('<td' + cls2 + '>' + n + '</td>');
      }
      h.push('<td class="stat" colspan="' + (generated ? 4 : 3) + '"></td></tr></tfoot>');
    }
    return { html: h.join(''), validation: validation, avail: avail, roster: roster, perDay: perDay, D: D, generated: generated, counts: counts, deltas: deltas, names: names };
  }
  ...
  function renderSchedule() {
    ...
    var brushes;
    if (!generated) {
      if (ui.brush !== 'R' && ui.brush !== 'S') ui.brush = 'R';
      brushes = [['R', '休息'], ['S', '特休']];
      step.innerHTML = '<span class="step-label"><strong>步驟一</strong> 選擇種類後點表格中的日期，再點一次可取消；不選擇則視為服從安排</span>';
    } else {
      if (ui.brush !== 'W' && ui.brush !== 'R' && ui.brush !== 'S') ui.brush = 'W';
      brushes = [['W', '上班'], ['R', '休息'], ['S', '特休']];
    ...
    if (!generated) {
      ...
      act.innerHTML =
        ...
        '<span class="sep"></span>' +
        '<select class="select-btn" id="export-select" aria-label="匯出格式">' +
    ...
    if (N && !generated) {
      ...
      if (a.totalAvail < a.totalSlots) list.push({ cls: 'warn', text: '本月總可出勤人日 ' + a.totalAvail + ' 少於需求 ' + a.totalSlots + '（' + t.perDay + ' 人 × ' + workable + ' 天），部分日子將無法排滿。' });
      if (N * 4 < t.perDay * 6) list.push({ cls: 'warn', text: '員工人數不足以維持上四休三：每週需 ' + (t.perDay * 6) + ' 人日，' + N + ' 人每週最多 ' + (N * 4) + ' 人日。' });
      // 個人選擇過多 / 過少（以本月每人平均可休的非週三天數為準）
      ...
      if (fairRest > 0) {
        ...
        t.roster.forEach(function (e) {
          ...
          if (!sel) return;
          if (sel > fairRest + 1) list.push({ cls: 'warn', text: t.names[e] + '：已選 ' + sel + ' 天休息，超過本月每人平均可休的約 ' + fr + ' 天（週三除外），其他人將需多上班。' });
          else if (sel < fairRest - 1) list.push({ cls: '', text: t.names[e] + '：只選了 ' + sel + ' 天休息，少於每人平均約 ' + fr + ' 天，其餘由系統安排。' });
        });
      }
      var pm = S.prevMonthOf(p.year, p.month);
      if (!prevData(key) && S.compareMonthKey(key, START_KEY) > 0 && db.months[S.monthKey(pm.year, pm.month)]) list.push({ cls: '', text: '上個月尚未生成排班表，跨月連續上班天數將無法檢查。' });
      if (!list.length) list.push({ cls: 'ok', text: '員工可在表格中點選本月想休的日子（休息或特休），不選擇則視為服從安排。準備好後按「生成排班表」。' });
    }
    if (generated) {
      ...
      if (!list.length) list.push({ cls: 'ok', text: '排班符合所有規則：每日 ' + t.perDay + ' 人出勤、無人連續上班超過 4 天、每週不超過 4 天、休息天數平均。' });
    }
  ...
  function buildExportDom(key) {
    ...
    root.innerHTML =
      ...
      '<p class="x-meta">' + (t.D - wedCount) + ' 個營業日 · 每日 ' + t.perDay + ' 人出勤' + (m.savedAt ? ' · 儲存於 ' + new Date(m.savedAt).toLocaleString('zh-TW', { hour12: false }) : '') + '</p>' +
      '<table class="grid">' + t.html + '</table>' +
      '<div class="legend"><span><span class="sw sw-W"></span>班＝上班</span><span><span class="sw sw-R"></span>休＝休息</span><span><span class="sw sw-S"></span>特＝特休</span></div>' +
  ...
  function exportXLSX(key) {
    ...
    aoa.push(foot);
    aoa.push([]);
    aoa.push(['說明：班＝上班、休＝休息、特＝特休；每日 ' + perDay + ' 人出勤；連續上班不超過 4 天。']);
    aoa.push(['© ' + p.year + ' KE FEI. All rights reserved.']);
    var ws = XLSX.utils.aoa_to_sheet(aoa);
    ws['!cols'] = [{ wch: 14 }].concat(new Array(D).fill({ wch: 7 })).concat(new Array(3).fill({ wch: 6 }));
    ws['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: Math.min(D + 3, 12) } }];
  ...
  function ask(opts) {
    return new Promise(function (resolve) {
      ...
      if (opts.input) { input.hidden = false; input.value = opts.input.value || ''; input.placeholder = opts.input.placeholder || ''; }
      ...
      ok.onclick = function () { done(opts.input ? input.value : true); };
      ...
      m.onkeydown = function (ev) {
        ...
        else if (ev.key === 'Enter' && opts.input) { ev.preventDefault(); ok.onclick(); }
      };
      setTimeout(function () { (opts.input ? input : ok).focus(); }, 0);
    });
  }
  var toastTimer = null;
  function toast(msg) {
  ...
  function renderSyncStatus(st) {
    ...
    if (st.mode === 'firebase') {
      ...
      el.title = '若要多人共用，請依 README 設定 Firebase';
    }
  }
  ...
  document.getElementById('emp-form').addEventListener('submit', function (ev) {
    ...
    addEmployee(input.value).then(function (added) {
      if (added) { input.value = ''; render(); }
      input.focus();
    });
  });
  document.getElementById('emp-list').addEventListener('click', function (ev) {
    ...
    var p = btn.classList.contains('del') ? removeEmployee(id) : btn.classList.contains('ren') ? renameEmployee(id) : Promise.resolve(false);
  ...
  Store.onRemoteChange(function (data) {
    db = data;
    render();
    toast('資料已由其他裝置更新');
  });
